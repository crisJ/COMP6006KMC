...
public class DummyAgent extends AgentImpl {
	...
	private double[][] historicASK;
	private double[][] historicBID;
	...
	public DummyAgent() {
		...
		DataRetriever oldData = new DataRetriever();
		
		historicASK = new double[12][55];
		historicBID = new double[12][55];
		historicASK = oldData.getDataASK();
		historicBID = oldData.getDataBID();
		
		if (historicASK[1][1]>0)
			log.fine("Historic Ask loaded" + historicASK[1][1]);
		else
			log.fine("I think there's an error in the load");
